import {
  GLYPH_APPEARANCE_PROFILE,
  GLYPH_BRIGHTNESS_TIER,
  PLAYER_ATTACK_VISUAL_ROLE,
  defineCombatVisualTheme,
  type CombatVisualThemeAuthoring,
} from './combatVisualTheme.ts'

export const PROTOTYPE_COMBAT_VISUAL_THEME_AUTHORING = {
  map: {
    canvasBackground: { tint: '#050505', alpha: 1 },
    backgroundGlyph: { tint: '#252525', alpha: 1 },
    obstacle: { tint: '#333333', alpha: 1 },
  },
  glyphAtlas: {
    sourceFillTint: '#ffffff',
    outlineTint: '#000000',
    outlineWidth: 5,
  },
  player: { tint: '#00ff88', alpha: 1 },
  playerSurvival: {
    healthDamage: {
      flash: { tint: '#fff0e6', alpha: 1 },
      durationMs: 170,
      pulseCount: 2.5,
      shakeDistance: 3.2,
      shakeCycles: 2.25,
      shakeVerticalRatio: 0.48,
      fragment: { tint: '#ff806d', alpha: 0.92 },
      fragmentCount: 6,
      fragmentDurationMs: 245,
      fragmentStaggerMs: 9,
      fragmentMinimumDistance: 13,
      fragmentMaximumDistance: 32,
      fragmentScale: 0.38,
    },
    shield: {
      base: { tint: '#8eeeff', alpha: 0.9 },
      glow: { tint: '#baf7ff', alpha: 0.62 },
      glyphOffsetX: 15,
      glyphScale: 0.76,
      glowRadiusX: 4,
      glowRadiusY: 11,
      glowBlurStrength: 5.5,
      glowBlurQuality: 2,
      glowBlurKernelSize: 7,
      glowBlurResolution: 0.75,
      glowBlurPadding: 12,
      hitShakeDurationMs: 115,
      hitShakeDistance: 2.8,
      hitShakeCycles: 2.25,
      hitShakeVerticalRatio: 0.42,
      depletionFadeDurationMs: 135,
      depletionOffsetDistance: 14,
      restoreDurationMs: 175,
      restoreOvershoot: 0.1,
    },
  },
  playerAttacks: {
    [PLAYER_ATTACK_VISUAL_ROLE.ASSISTED_PROJECTILE]: {
      core: { tint: '#66ddff', alpha: 1 },
      accent: { tint: '#66ddff', alpha: 0.72 },
    },
    [PLAYER_ATTACK_VISUAL_ROLE.FLAMETHROWER]: {
      core: { tint: '#ffdd33', alpha: 1 },
      accent: { tint: '#ff7a18', alpha: 0.9 },
    },
    [PLAYER_ATTACK_VISUAL_ROLE.ORBIT_ENERGY]: {
      core: { tint: '#66eeff', alpha: 1 },
      accent: { tint: '#66eeff', alpha: 0.72 },
    },
  },
  glyphBrightnessTiers: {
    [GLYPH_BRIGHTNESS_TIER.ORDINARY]: {
      id: GLYPH_BRIGHTNESS_TIER.ORDINARY,
      activeGain: 1,
      damagedGainFloor: 0.72,
      impactGain: 1.42,
      huskGain: 0.45,
      healthyAlpha: 1,
      damagedAlphaFloor: 0.32,
      impactAlphaFloor: 0.86,
      huskAlpha: 0.18,
    },
    [GLYPH_BRIGHTNESS_TIER.BOSS]: {
      id: GLYPH_BRIGHTNESS_TIER.BOSS,
      activeGain: 1.12,
      damagedGainFloor: 0.8,
      impactGain: 1.55,
      huskGain: 0.52,
      healthyAlpha: 1,
      damagedAlphaFloor: 0.4,
      impactAlphaFloor: 0.9,
      huskAlpha: 0.22,
    },
  },
  glyphAppearances: {
    [GLYPH_APPEARANCE_PROFILE.ZOMBIE]: {
      id: GLYPH_APPEARANCE_PROFILE.ZOMBIE,
      brightnessTierId: GLYPH_BRIGHTNESS_TIER.ORDINARY,
      durabilityBaseTints: ['#297d42'],
      eyeDurabilityBaseTints: null,
      impactBaseTint: '#236b38',
      huskBaseTint: '#236b38',
    },
    [GLYPH_APPEARANCE_PROFILE.BONE]: {
      id: GLYPH_APPEARANCE_PROFILE.BONE,
      brightnessTierId: GLYPH_BRIGHTNESS_TIER.ORDINARY,
      durabilityBaseTints: ['#918f8a'],
      eyeDurabilityBaseTints: null,
      impactBaseTint: '#68645a',
      huskBaseTint: '#68645a',
    },
    [GLYPH_APPEARANCE_PROFILE.BAT]: {
      id: GLYPH_APPEARANCE_PROFILE.BAT,
      brightnessTierId: GLYPH_BRIGHTNESS_TIER.ORDINARY,
      durabilityBaseTints: ['#8232a4'],
      eyeDurabilityBaseTints: null,
      impactBaseTint: '#552277',
      huskBaseTint: '#552277',
    },
    [GLYPH_APPEARANCE_PROFILE.SLIME_BOSS]: {
      id: GLYPH_APPEARANCE_PROFILE.SLIME_BOSS,
      brightnessTierId: GLYPH_BRIGHTNESS_TIER.BOSS,
      durabilityBaseTints: ['#268d48', '#267a42'],
      eyeDurabilityBaseTints: ['#805f1e', '#9a7425'],
      impactBaseTint: '#267a42',
      huskBaseTint: '#1f6637',
    },
  },
  drops: {
    experience: {
      scale: 0.7,
      fresh: { tint: '#e0b83e', alpha: 0.92 },
      settled: { tint: '#765d1c', alpha: 0.72 },
      flash: { tint: '#c99d26', alpha: 0.88 },
      freshDurationMs: 650,
      settleTransitionMs: 450,
      flashIntervalMs: 3_200,
      flashDurationMs: 220,
    },
    other: { tint: '#b08a2e', alpha: 0.8 },
  },
  effects: {
    spreadFeedbackAlpha: 0.72,
    spreadFeedbackScaleBonus: 0.08,
    spreadFeedbackDurationMs: 140,
    topologyTransfer: {
      tint: '#5ea8b8',
      alpha: 0.62,
      stepIntervalMs: 45,
      pulseDurationMs: 120,
    },
    runModifiers: {
      composition: {
        maximumOffset: 5.8,
        maximumRotation: 0.05,
      },
      volatile: {
        sourceClamp: {
          attackDurationMs: 8,
          holdDurationMs: 6,
          settleDurationMs: 28,
          minimumScale: 0.74,
        },
        release: {
          tint: '#e39b56',
          alpha: 0.78,
          delayMs: 14,
          attackDurationMs: 6,
          holdDurationMs: 4,
          settleDurationMs: 24,
          startDistance: 4,
          endDistance: 22,
          glyphScale: 0.34,
          characters: ['|', '-', '|', '-'],
        },
        neighborJolt: {
          delayMs: 14,
          attackDurationMs: 6,
          holdDurationMs: 4,
          settleDurationMs: 24,
          maximumOffset: 3.5,
          maximumRotation: 0.03,
        },
        clusterBurst: {
          tint: '#fff6d4',
          alpha: 0.94,
          attackDurationMs: 12,
          holdDurationMs: 50,
          settleDurationMs: 100,
          maximumSpawnStaggerMs: 10,
          maximumLifetimeVariationMs: 100,
          minimumClusterCount: 3,
          maximumClusterCount: 5,
          minimumPointsPerCluster: 4,
          maximumPointsPerCluster: 7,
          minimumClusterCenterDistance: 5,
          maximumClusterCenterDistance: 14,
          maximumClusterAngleJitterRadians: 0.42,
          pointScatterRadius: 10,
          outwardDistance: 10,
          minimumGlyphScale: 0.5,
          maximumGlyphScale: 1,
          maximumActivePointCount: 5_000,
          characters: ['.'],
          centerHighlight: {
            tint: '#ffffff',
            alpha: 0.98,
            delayMs: 5,
            attackDurationMs: 9,
            holdDurationMs: 18,
            settleDurationMs: 42,
            glyphScale: 0.44,
            character: '*',
          },
        },
      },
      disconnected: {
        ambient: {
          attackDurationMs: 24,
          holdDurationMs: 18,
          settleDurationMs: 72,
          intervalMs: 1_800,
          maximumSpacingOffset: 2.8,
          maximumJitterOffset: 1.15,
          maximumRotation: 0.026,
        },
        hitShake: {
          attackDurationMs: 32,
          holdDurationMs: 22,
          settleDurationMs: 86,
          maximumOffset: 5.5,
          residualOffsetRatio: 0.34,
          maximumRotation: 0.038,
        },
      },
      overload: {
        compression: {
          attackDurationMs: 45,
          holdDurationMs: 30,
          settleDurationMs: 90,
          parallelScale: 0.7,
          perpendicularScale: 1.08,
        },
        shockwave: {
          tint: '#c9a1f2',
          alpha: 0.76,
          attackDurationMs: 35,
          holdDurationMs: 20,
          settleDurationMs: 80,
          startRadius: 7,
          endRadius: 38,
          particleCount: 8,
          glyphScale: 0.32,
          characters: ['(', ')', '-', '|', '/', '\\', '<', '>'],
        },
        crackedSurface: {
          alphaMultiplier: 0.96,
          maximumFragmentOffset: 5,
          maximumFragmentRotation: 0.1,
          fragmentBrightnessGains: [0.6, 1, 1.4],
        },
      },
    },
  },
} satisfies CombatVisualThemeAuthoring

export const PROTOTYPE_COMBAT_VISUAL_THEME = defineCombatVisualTheme(
  PROTOTYPE_COMBAT_VISUAL_THEME_AUTHORING,
)
